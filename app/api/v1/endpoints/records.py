...
from app.schemas.financial_record import FinancialRecordCreate, FinancialRecordUpdate, FinancialRecordResponse
...
def read_records(
    ...
    records = record_service.list_records(db, skip=skip, limit=limit, type=type, category=category)
    return records
...
def create_record(
    *,
    ...
    record = record_service.create_record(db, record_in=record_in, user_id=current_user.id)
    return record
...
def update_record(
    *,
    ...
    try:
        record = record_service.update_record(db, record_id=record_id, obj_in=record_in)
        return record
